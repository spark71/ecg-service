...
class TECGDataModule484(pl.LightningDataModule):
    def __init__(self, data_dir, batch_size, phase='train', seed=0):
        super(TECGDataset484, self).__init__()
        self.data_dir = data_dir
    ...
    def prepare_data(self):
        self.base_img_paths = glob.glob(os.path.join(self.data_dir, '00001.npy'))
        self.style_img_paths = glob.glob(os.path.join(self.data_dir, '00001.npy'))
...
class TECGDataset484(Dataset):
    def __init__(self, base_img_paths, style_img_paths, phase='train'):
        ...
        path_data_ch1 = np.load(self.base_img_paths[0])
        label = np.load(self.style_img_paths[0])
        self.data_ch1 = path_data_ch1["noisy_sig"]
        ...
        self.inputt = torch.tensor(self.data).float()
        self.labels = torch.tensor(self.data2).float()
        print("{0} samples with {1} channels each and {2} classes".format(self.num_samples, self.num_channels,
                                                                          self.labels.unique().numel()))
